...
#SBATCH --output=slurm/pascal/mask2former/mask2former_pascal_untargeted_%a_%A.out
#SBATCH --array=0-2
...
then
    ...
    do 
        ...
        epsilon="${epsilons[i]}"
        alpha="${alphas[i]}"
        python tools/test.py ../configs/mask2former/mask2former_swin-s_8xb2-160k_voc12aug-512x512.py /pfs/work7/workspace/scratch/ma_dschader-team_project_fss2024/benchmarking_robustness/semantic_segmentation/work_dirs/mask2former_swin-s_8xb2-160k_voc12aug-512x512/best_mIoU_iter_155000.pth --cfg-options model.perform_attack=True model.attack_cfg.name=${name} model.attack_cfg.norm=${norm} model.attack_cfg.alpha=${alpha} model.attack_cfg.epsilon=${epsilon} model.attack_cfg.iterations=${iterations} --work-dir ../aa_workdir/pascal/mask2former/mask2former_swin-s_8xb2-160k_voc12aug-512x512/attack_${name}/norm_${norm}/iterations_${iterations}/epsilon_${epsilon}/alpha_${alpha} --show-dir ../aa_workdir/pascal/mask2former/mask2former_swin-s_8xb2-160k_voc12aug-512x512/attack_${name}/norm_${norm}/iterations_${iterations}/epsilon_${epsilon}/alpha_${alpha}/show_dir
    done
